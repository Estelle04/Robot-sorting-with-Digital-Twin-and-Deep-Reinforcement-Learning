...
void sendDebugToNodeRED(String message);
...
void sendAngleToProcessing(JointWaypoint joint_states_vector)
{
  ...
  {
    // changed from the original to send relevant data to node-red
...
void fromProcessing(OpenManipulator *open_manipulator, String data)
{
  ...
  String debugMessage = "Received command: ";
  for (int i = 0; i < 50; i++) {
    if (cmd[i] == "") break;
    debugMessage += cmd[i];
    debugMessage += " ";
  }
  sendDebugToNodeRED(debugMessage);
...
void playProcessingMotion(OpenManipulator *open_manipulator)
{
  if(!open_manipulator->getMovingState() && processing_motion_state)
  {
    if(motion_way_point_buf.size() == 0){
      sendDebugToNodeRED("No motion waypoints available");
      return;
    }
    ...
    if(hand_motion_cnt >= motion_way_point_buf.size())
    {
      ...
      if(!hand_motion_repeat_state){
        processing_motion_state = false;
        sendDebugToNodeRED("Motion playback completed");
    }
  }
}
}
...
// Define the function
void sendDebugToNodeRED(String message) {
  Serial.print("DEBUG:"); // Prefix for debug messages
